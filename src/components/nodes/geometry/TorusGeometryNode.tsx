...
interface TorusGeometryData {
  radius: number;
  tube: number;
  radialSegments: number;
  tubularSegments: number;
  arc: number;
  onUpdate?: (id: string, data: Partial<TorusGeometryData>) => void;
...
const TorusGeometryNode = ({ data, id }: NodeProps<TorusGeometryData>) => {
  const handleChange = (field: keyof TorusGeometryData, value: string) => {
    if (data.onUpdate) {
  ...
  return (
    <div className="node-box" style={{
      ...
      <div style={{ padding: '5px' }}>
        <h4 style={{ margin: '0 0 8px 0' }}>TorusGeometry</h4>
...
export default memo(TorusGeometryNode);
